...
def import_folder(path):
    ...
    for _,__,image_files in os.walk(path):
        ...
        for image in image_files:
            ...
            image_surf = pg.image.load(full_path).convert_alpha() # inflate 
...
class Player(pg.sprite.Sprite):
    ...
    def __init__(self) :
        super().__init__()
        self.player_walk_test = import_folder('graphics\player/run_right')
        self.player_index = 0
...
class Obstacles(pg.sprite.Sprite):
    ...
    def __init__(self,type):
        ...
        if type == 'fly':
            ...
            self.frames = import_folder('graphics\enemy/run_left')
            y_pos = 300
...
def display_score():
    ...
    score_surf = test_font.render(f'Score : {int(curr_time/1000)}',False,(64,64,64))
...
pg.display.set_caption("Runner") # name of game
clock = pg.time.Clock()
...
#intro
game_name = test_font.render("Pirate Mario",False,(245, 56, 56))
game_name_rect = game_name.get_rect(center = (400,70))

game_msg = test_font.render('Press  Space  to  run' , False , (245, 56, 56))
...
while True:
    ...
    if game_active :
        ...
        # for score msg
        score_msg = test_font.render(f'Your Score:{score}',False,(245, 56, 56))
